    ...
    void process_tree(tree_iter_t tree,SpinSystem* ss) {
        ...
        for(tree_iter_t j=tree->children.begin();j!=tree->children.end();++j) {
            if(j->value.id()==total_shielding_tensorID || j->value.id()==total_tensorID) {
                ...
                if(it == spin_map.end()) { //If the spin dosn't already exist
                    //Create the spin
                    length x,y,z;
                    x=getDouble(x_coord) / Angstroms;
                    y=getDouble(y_coord) / Angstroms;
                    z=getDouble(z_coord) / Angstroms;
                    long element=getElementBySymbol(getString(atom).c_str());
                    if(element == -1) {
                        throw runtime_error("Unknown Element "+getString(atom));
                    }
                    Spin* spin = new Spin(Vector3d(x,y,z),label,element);
        ...
        for(tree_iter_t j=tree->children.begin();j!=tree->children.end();++j) {
            if(j->value.id()==total_tensorID || j->value.id()==total_shielding_tensorID) {
                ...
                //Eigenvalues
                energy xx,yy,zz;
                unit u = j->value.id()==total_tensorID ? Joules : MHz;
                xx = getDouble(eigsys_xx->children.begin()+1) / u;
                yy = getDouble(eigsys_yy->children.begin()+1) / u;
                zz = getDouble(eigsys_zz->children.begin()+1) / u;

                //Eigenvectors
				Matrix3d mat = MakeMatrix3d(getDouble(eigsys_xx->children.begin()+3),
											getDouble(eigsys_xx->children.begin()+4),
											getDouble(eigsys_xx->children.begin()+5),

											getDouble(eigsys_yy->children.begin()+3),
											getDouble(eigsys_yy->children.begin()+4),
											getDouble(eigsys_yy->children.begin()+5),

											getDouble(eigsys_zz->children.begin()+3),
											getDouble(eigsys_zz->children.begin()+4),
											getDouble(eigsys_zz->children.begin()+5));
                Orientation o(mat);
                Interaction::Type type=j->value.id()==total_tensorID  ?
                    Interaction::CUSTOM_LINEAR :
                    Interaction::SHIELDING;
				
                Interaction* inter = new Interaction(Eigenvalues(xx,yy,zz,o),type,spin);

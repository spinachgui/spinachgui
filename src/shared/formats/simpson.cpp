...
void SIMPSONLoader::SaveFile(const SpinSystem* ss,const char* filename) const {
  ...
  for(long i=0;i<count;i++){
    ...
    long neutronN=spin->GetIsotopes()[0];
  ...
  for(long i=0;i<count;i++) {
    Spin* spin=spins[i];
    long element=spin->GetElement();
    long isotope=spin->GetIsotopes()[0]+element;
  ...
  for(long i=0;i<count;i++) {
      ...
      Spin* spin=spins[i];
      vector<Interaction*> inters=spin->GetInteractionsOnce();
      
      for(vector<Interaction*>::iterator it=inters.begin();
          it != inters.end();++it) {
          if((*it)->GetSubType() == Interaction::ST_DIPOLAR) {
              fout << "  dipole " 
                   << i << " " 
